...
const PORT = 28330 + Math.floor(Math.random() * 200)
const ENDPOINT = `http://127.0.0.1:${PORT}`
// mock-om-core does not enforce auth (token is empty). Real om-core under
// spec 19 binds a UDS — fs perm is the auth boundary, not bearer. The shape
// of AUTH matches what production code receives in legacy TCP / external
// mode. UDS-mode targets carry `transport: 'uds'` and `target: socket_path`.
const AUTH: import('../../core/om-core/transport').OmCoreAuth = {
  transport: 'tcp',
...
describe('KogCat full chain (mock om-core)', () => {
  it('engine reachable + prompts cached on disk by hash', async () => {
    ...
    const health = await fetch(`${ENDPOINT}/healthz`).then((r) => r.json())
  ...
  it('calibrate honors timeout when om-core hangs', async () => {
    // Point at a closed port — fetch should fail fast and return null.
    const r = await calibrate({
      auth: { transport: 'tcp', target: '127.0.0.1:1', token: '' },
      text: 'whatever',
      source: 'chat_response',
      timeoutMs: 200,
    })
