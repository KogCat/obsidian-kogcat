      ...
      <div className="view-header">
        <div className="view-header-title-container mod-at-start">
          ...
          <div className="view-actions">
            <div className="cc-diff-navigation">
              <button
...
>(({ block: part, onAcceptIncoming, onAcceptCurrent, onAcceptBoth }, ref) => {
  if (part.type === 'unchanged') {
    return (
      <div className="cc-diff-block">
        <div style={{ width: '100%' }}>{part.value}</div>
      </div>
    )
  } else if (part.type === 'modified') {
    return (
      <div className="cc-diff-block-container" ref={ref}>
        {part.originalValue && part.originalValue.length > 0 && (
          <div className="cc-diff-block removed">
            <div style={{ width: '100%' }}>{part.originalValue}</div>
          </div>
        )}
        {part.modifiedValue && part.modifiedValue.length > 0 && (
          <div className="cc-diff-block added">
            <div style={{ width: '100%' }}>{part.modifiedValue}</div>
          </div>
        )}
        <div className="cc-diff-block-actions">
          <button onClick={onAcceptIncoming} className="cc-accept">
            Accept Incoming
          </button>
          <button onClick={onAcceptCurrent} className="cc-exclude">
            Accept Current
          </button>
