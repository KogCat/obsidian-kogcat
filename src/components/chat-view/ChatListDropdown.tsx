...
function TitleInput({
  ...
  return (
    <input
      ...
      type="text"
      value={value}
      className="cc-chat-list-dropdown-item-title-input"
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <div className="cc-chat-list-dropdown-item-title">{title}</div>
      )}
      <div className="cc-chat-list-dropdown-item-actions">
        <button
          onClick={(e) => {
            e.stopPropagation()
            onStartEdit()
          }}
          className="clickable-icon cc-chat-list-dropdown-item-icon"
        >
          <Pencil />
        </button>
        <button
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild>
        <button className="clickable-icon" aria-label={t('history.trigger')}>
          {children}
        </button>
      </Popover.Trigger>
      ...
      <Popover.Portal>
        <Popover.Content
          className="cc-popover cc-chat-list-dropdown-content"
          onKeyDown={handleKeyDown}
        >
          <ul>
            {chatList.length === 0 ? (
              <li className="cc-chat-list-dropdown-empty">
                {t('history.empty')}
              </li>
            ) : (
