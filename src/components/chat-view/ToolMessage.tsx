...
const ToolMessage = memo(function ToolMessage({
  ...
  return (
    <div className="cc-toolcall-container">
      {message.toolCalls.map((toolCall, index) => (
        <div
          key={toolCall.request.id}
          className={clsx(index > 0 && 'cc-toolcall-border-top')}
        >
          <ToolCallItem
...
function ToolCallItem({
  ...
  const tAbort = t('tool.abort')
  const tReject = t('tool.reject')
  const {
    handleToolCall,
  ...
  return (
    <div className="cc-toolcall">
      <div
        onClick={() => setIsOpen(!isOpen)}
        className="cc-toolcall-header"
      >
        <div className="cc-toolcall-header-icon">
          {isOpen ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
        </div>
        <div className="cc-toolcall-header-content">
          <span>{STATUS_LABELS[response.status] || 'Unknown'}</span>
          <span>&nbsp;&nbsp;</span>
          <span className="cc-toolcall-header-tool-name">
            {serverName ? `${serverName}:${toolName}` : toolName}
          </span>
        </div>
        <div className="cc-toolcall-header-icon cc-toolcall-header-icon--status">
          <StatusIcon status={response.status} />
        </div>
      </div>
      {isOpen && (
        <div className="cc-toolcall-content">
          <div className="cc-toolcall-content-section">
            <div>Parameters:</div>
      ...
      {(response.status === ToolCallResponseStatus.PendingApproval ||
        ...
        <div className="cc-toolcall-footer">
          {response.status === ToolCallResponseStatus.PendingApproval && (
            <div className="cc-toolcall-footer-actions">
              <SplitButton
                primaryText="Allow"
                onPrimaryClick={() => {
                  handleToolCall()
          ...
          {response.status === ToolCallResponseStatus.Running && (
            <div className="cc-toolcall-footer-actions">
              <button onClick={handleAbort}>{tAbort}</button>
            </div>
          )}
        </div>
      )}
