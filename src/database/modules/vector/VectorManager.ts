...
export class VectorManager {
  ...
  async updateVaultIndex(
    ...
    if (options.reindexAll) {
      ...
      filesToIndex = await this.getFilesToIndex({
        candidateFiles: this.getMarkdownFilesByPath(filePaths),
        embeddingModel: embeddingModel,
        excludePatterns: options.excludePatterns,
        includePatterns: options.includePatterns,
      })
    ...
    const textSplitter = RecursiveCharacterTextSplitter.fromLanguage(
      'markdown',
      {
        ...
        // TODO: token-based chunking once WASM tiktoken is fast enough.
      },
    )
...
Please report this issue to the developer if it persists.`,
  ...
  private async getFilesToIndex({
    ...
    let filesToIndex = candidateFiles ?? this.app.vault.getMarkdownFiles()
    ...
    // TODO: batch repository query to avoid N round-trips here.
  ...
  private getMarkdownFilesByPath(filePaths: string[]): TFile[] {
    return filePaths
