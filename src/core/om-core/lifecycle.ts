...
// om-core uses a series-reset api_minor model — CHANGELOG 0.32.0:
...
// series; we anchor on a minimum (major, minor) series parsed from the
// healthz `version` field, plus a minimum api_minor *within* that series.
//
// Series (0, 32) is the first series after the 0.31.0 calibrate/log BREAK
// and the 0.32.0 ontology-delta BREAK, both of which this wrapper assumes.
const MIN_REQUIRED_SERIES: readonly [number, number] = [0, 34]
const MIN_REQUIRED_API_MINOR = 0
...
function checkCompat(version: string | undefined, apiMinor: number | undefined): CompatCheck {
  ...
  if (series[0] < MIN_REQUIRED_SERIES[0] ||
      (series[0] === MIN_REQUIRED_SERIES[0] && series[1] < MIN_REQUIRED_SERIES[1])) {
    return {
      ok: false,
      message:
        `om-core version=${version} (series ${series[0]}.${series[1]}.x) but ` +
        `Obsidian wrapper requires >= ${reqSeries}. Upgrade the KogCat engine.`,
    }
  }
  if (series[0] === MIN_REQUIRED_SERIES[0] && series[1] === MIN_REQUIRED_SERIES[1] &&
...
export class OmCoreLifecycle {
  ...
  // External mode: caller owns the process. TCP only.
  async attachExternal(port: number): Promise<void> {
    ...
    const transport: OmCoreTransport = {
      kind: 'tcp',
      host: '127.0.0.1',
      port,
    }
    const auth = transportToAuth(transport, token)
    const healthy = await waitForHealth(auth)
    if (!healthy) {
      this.setStatus({
        kind: 'failed',
        message: `External om-core not reachable on port ${port}`,
      })
      return
    }
    const compat = checkCompat(healthy.version, healthy.api_minor)
    if (!compat.ok) {
  ...
  async directSpawn(binaryPath: string, opts: OmCoreLifecycleOptions): Promise<void> {
    if (!directSpawnEnabled()) {
      this.setStatus({
        kind: 'failed',
        message:
    ...
    const cfg = await waitForServerJson(spawnTimeMs, SERVER_JSON_POLL_TIMEOUT_MS)
    if (!cfg) {
      this.setStatus({
        kind: 'failed',
        message:
...
async function transportProbeAlive(
  cfg: { transport?: string; socket_path?: string; port?: number },
): Promise<boolean> {
  ...
  if (typeof cfg.port === 'number') {
    return tcpListenerAlive('127.0.0.1', cfg.port)
  }
  return false
}
...
function tcpListenerAlive(host: string, port: number): Promise<boolean> {
  return new Promise((resolve) => {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const net = require('net') as typeof import('net')
  ...
  for (let i = 0; i < HEALTH_RETRIES; i++) {
    try {
      ...
      if (res.status >= 200 && res.status < 300) {
        const body = (res.json ?? {}) as { version?: string; api_minor?: number }
