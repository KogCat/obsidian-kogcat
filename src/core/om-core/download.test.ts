...
import {
  ...
  aggregateManifestUrlsForDownload,
  channelUrlsForDownload,
  installBundle,
  mirrorCandidates,
  pickFromChannel,
} from './download'
...
const REQUIRED_SPEC_FOR_TEST = '22'
...
describe('pickFromChannel', () => {
  it('picks the highest spec-compatible release', () => {
    const ch: Channel = {
      ...
      releases: [
        {
          om_core_version: '0.37.0',
          spec: REQUIRED_SPEC_FOR_TEST,
          targets: { [TRIPLE]: mkEntry('0.37.0') },
        },
        {
          om_core_version: '0.36.17',
          spec: REQUIRED_SPEC_FOR_TEST,
          targets: { [TRIPLE]: mkEntry('0.36.17') },
        },
      ],
    }
    expect(pickFromChannel(ch, TRIPLE)?.version).toBe('0.37.0')
  })
  ...
  it('skips releases whose spec does not match REQUIRED_SPEC', () => {
    const ch: Channel = {
      ...
      releases: [
        {
          om_core_version: '0.40.0',
          spec: '20',
          targets: { [TRIPLE]: mkEntry('0.40.0') },
        },
      ],
    }
      ...
      releases: [
        {
          om_core_version: '0.37.0',
          spec: REQUIRED_SPEC_FOR_TEST,
          targets: { 'x86_64-pc-windows-msvc': mkEntry('0.37.0') },
        },
      ],
    }
    expect(pickFromChannel(ch, TRIPLE)).toBeNull()
      ...
      releases: [
        {
          om_core_version: '0.37.0',
          spec: REQUIRED_SPEC_FOR_TEST,
          targets: { [TRIPLE]: mkEntry('0.37.0', 'tar.xz') },
        },
      ],
    }
...
describe('mirror URL order', () => {
  it('tries Ali OSS channel before GitHub canonical channel', () => {
    const urls = channelUrlsForDownload()
    expect(urls[0]).toBe(
      'https://kogcat.oss-cn-beijing.aliyuncs.com/om-core/channel/channel.json',
    )
    expect(urls[1]).toContain('github.com/KogCat/om-core-binaries')
  })

  it('tries Ali OSS aggregate manifest before GitHub canonical release', () => {
    const urls = aggregateManifestUrlsForDownload('0.36.10')
    expect(urls[0]).toBe(
      'https://kogcat.oss-cn-beijing.aliyuncs.com/om-core/v0.36.10/aggregate-manifest.json',
    )
    expect(urls[1]).toContain('/releases/download/v0.36.10/')
  })
})
...
describe('installBundle', () => {
  ...
  it('extracts a flat-root bundle and yields a runnable om-core-bin', async () => {
    ...
    await installBundle(archive, bundleDir, 'tar.xz')
  ...
  it('rejects an archive with no om-core-bin executable', async () => {
    ...
    const bundleDir = path.join(workDir, 'bin', '0.36.0', 'bundle')
    await expect(installBundle(archive, bundleDir, 'tar.xz')).rejects.toThrow(
...
describe('mirrorCandidates', () => {
  const asset = 'om-core-bin-x86_64-pc-windows-msvc.zip'

  it('leads with the resolved (race-winner) url, then derives the other mirror', () => {
    // GitHub channel won the race → its asset url leads; Ali derived as fallback.
    const entry = {
      url: `https://github.com/KogCat/om-core-binaries/releases/download/v0.36.34/${asset}`,
      asset_name: asset,
    }
    expect(mirrorCandidates(entry, '0.36.34')).toEqual([
      entry.url,
      `https://kogcat.oss-cn-beijing.aliyuncs.com/om-core/v0.36.34/${asset}`,
    ])
  })

  it('when the winner is the Ali mirror, GitHub is the derived fallback', () => {
    const ali = `https://kogcat.oss-cn-beijing.aliyuncs.com/om-core/v0.36.34/${asset}`
    expect(
      mirrorCandidates({ url: ali, asset_name: asset }, '0.36.34'),
    ).toEqual([
      ali,
      `https://github.com/KogCat/om-core-binaries/releases/download/v0.36.34/${asset}`,
    ])
  })
})
